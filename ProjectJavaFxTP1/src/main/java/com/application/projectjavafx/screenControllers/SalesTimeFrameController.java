...
@Component
@FxmlView("salesTimeFrame.fxml")
public abstract class SalesTimeFrameController implements ControlledScreen {
...
//    Report report = new Report(from, to, "SalesTimeframe");
//    ArrayList<Operation> salesArrayList = report.getSalesTimeframe(from, to);
//    ArrayList<SalesAndPurchasesList> salesListArrayList = new ArrayList<>();
//
//    for (Operation operation : salesArrayList) {
//      SalesAndPurchasesList item = new SalelosAndPurchasesList(operation);
//      salesListArrayList.add(item);
//    }
//    ObservableList<SalesAndPurchasesList> list = FXCollections.observableArrayList(salesListArrayList);
  ...
  public void issueReport(ActionEvent event) {
    ...
    try {
      ...
      if (isValidDate(startDate)) {
        ...
        invalidEndDateLabel.setVisible(!isValidDate(endDate));
      }
    }
